...
function model = create3DoFRobotModel(rotInertia_arr, mass, L)
    ...
    % disp(L)
    % L = [-0.1, L];                  % Link length (adjustable)
    % disp(L)
    ...
    model.jtype{1} = 'Rz';
    model.Xtree{1} = eye(6);
    model.appearance.body{1} = {{'cyl', [0,0,.05;0,0,-.05],.05},{'cyl', [0,0,0;0,0,L(1)],0.02}};
    model.I{1} = mcI(mass(1), [0, 0, L(1)], rotInertia{1});
    ...
    model.Xtree{4} = plux(eye(3), [0, 0, L(3)]);
