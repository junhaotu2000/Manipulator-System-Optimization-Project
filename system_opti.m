...
CombinedOptimization(opti_fun)




function CombinedOptimization(opti_fun)
    % CombinedOptimization runs the mechanical design optimization via GA and then
    % uses the resulting parameters as inputs to the robot planning optimization.
    %
    % The overall workflow is:
    %   1. Run mechanical design optimization (GA) to obtain link inertias and masses.
    %   2. Convert the GA outputs to the proper format for the robot planning module.
    %   3. Run the CasADi-based robot planning optimization using these parameters.
    %
    % Outputs (displayed and plotted):
    %   - Mechanical design outputs (rotational inertia array and mass vector).
    %   - Robot planning optimization: joint angle and torque trajectories.
